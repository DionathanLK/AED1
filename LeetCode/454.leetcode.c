...
int fourSumCount( int *nums1, int nums1Size, int *nums2, int nums2Size,
	...
	int n = nums1Size;
	int totalsize = n * n;
	int* totalsum12 = (int*) malloc ( totalsize * sizeof( int* ) );
	int* totalsum34 = (int*) malloc ( totalsize * sizeof( int* )	);

	int num = 0;

	for ( int i = 0; i < n; i++ ) {
		for ( int j = 0; j < n; j++ ) {
			totalsum12[num] = totalsum12[i] + totalsum12[j];
			totalsum34[num] = totalsum34[i] + totalsum34[j];
			num++;

			}
		}

			qsort ( totalsum12, totalsize, sizeof(int), comp );
			qsort ( totalsum34, totalsize, sizeof(int), comp );
	...
	while ( k < totalsize && j ) {
		int sum = totalsum12[k] + totalsum34[j];
		...
		while ( k + 1 < totalsize && totalsum12[k] == totalsum12[k + 1] ) {
                count12++;
                k++;

            }

        while ( j - 1 >= 0 && totalsum34[j] == totalsum34[j - 1] ) {
                count34++;
                j--;
				
            }
            
            totaltuples += (count12 * count34);
            k++;
            j--;
